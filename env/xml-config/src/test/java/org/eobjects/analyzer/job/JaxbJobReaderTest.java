...
public class JaxbJobReaderTest extends TestCase {
	...
	public void testReadComponentNames() throws Exception {
		...
		AnalysisJob job = reader.read(new FileInputStream(new File(
				"src/test/resources/example-job-merged-outcome.xml")));

		assertEquals(1, job.getAnalyzerJobs().size());
		assertEquals("analyzer_1", job.getAnalyzerJobs().iterator().next()
				.getName());

		assertEquals(2, job.getFilterJobs().size());
		assertEquals("single_word_1", job.getFilterJobs().iterator().next()
				.getName());

		assertEquals(1, job.getTransformerJobs().size());
		assertEquals("email_std_1", job.getTransformerJobs().iterator().next()
				.getName());

		assertEquals(2, job.getMergedOutcomeJobs().size());
		assertEquals("merge_1", job.getMergedOutcomeJobs().iterator().next()
				.getName());
	}

	public void testReadMetadataFull() throws Exception {
		JobReader<InputStream> reader = new JaxbJobReader(conf);
		AnalysisJobMetadata metadata = reader.readMetadata(new FileInputStream(
				new File("src/test/resources/example-job-metadata.xml")));
		...
		assertEquals("An example job with complete metadata",
				metadata.getJobDescription());
		assertEquals("1.1", metadata.getJobVersion());
		assertEquals("[PUBLIC.PERSONS.FIRSTNAME, PUBLIC.PERSONS.LASTNAME]",
	...
	public void testReadMetadataNone() throws Exception {
		JobReader<InputStream> reader = new JaxbJobReader(
				new AnalyzerBeansConfigurationImpl());
		AnalysisJobMetadata metadata = reader.readMetadata(new FileInputStream(
				new File("src/test/resources/example-job-valid.xml")));
		...
		assertEquals("my database", metadata.getDatastoreName());
		assertEquals(
				"[PUBLIC.EMPLOYEES.FIRSTNAME, PUBLIC.EMPLOYEES.LASTNAME, PUBLIC.EMPLOYEES.EMAIL]",
				metadata.getSourceColumnPaths().toString());
	...
	public void testSimpleFilter() throws Exception {
		...
		AnalysisJobBuilder jobBuilder = reader.create(new File(
				"src/test/resources/example-job-simple-filter.xml"));
		...
		assertEquals(
				1,
				res3.getCrosstab().where("Column", "FIRSTNAME")
						.where("Measures", "Min words").get());
		assertEquals(
				2,
				res3.getCrosstab().where("Column", "FIRSTNAME")
						.where("Measures", "Max words").get());
		...
		assertEquals(1, res1.getSingleValueDistributionResult()
				.getUniqueCount());
		assertEquals("ValueCountList[[]]", res1
		...
		assertEquals(18, res2.getSingleValueDistributionResult()
				.getUniqueCount());
		assertEquals("ValueCountList[[[Leslie->2], [Gerard->2]]]", res2
				.getSingleValueDistributionResult().getTopValues().toString());
	}
	...
	public void testInvalidRead() throws Exception {
		JaxbJobReader factory = new JaxbJobReader(
				new AnalyzerBeansConfigurationImpl());
		try {
	...
	public void testValidJob() throws Exception {
		...
		assertEquals("MetaModelInputColumn[PUBLIC.EMPLOYEES.FIRSTNAME]",
				sourceColumns.get(0).toString());
		assertEquals("MetaModelInputColumn[PUBLIC.EMPLOYEES.LASTNAME]",
				sourceColumns.get(1).toString());
		assertEquals("MetaModelInputColumn[PUBLIC.EMPLOYEES.EMAIL]",
	...
	public void testUsingSourceColumnMapping() throws Throwable {
		...
		AnalysisJobMetadata metadata = reader.readMetadata(new FileInputStream(
				new File("src/test/resources/example-job-valid.xml")));
		SourceColumnMapping sourceColumnMapping = new SourceColumnMapping(
				metadata.getSourceColumnPaths());
		assertFalse(sourceColumnMapping.isSatisfied());
		assertEquals(
				"[PUBLIC.EMPLOYEES.EMAIL, PUBLIC.EMPLOYEES.FIRSTNAME, PUBLIC.EMPLOYEES.LASTNAME]",
		...
		assertEquals(
				"[MetaModelInputColumn[PUBLIC.CUSTOMERS.CONTACTFIRSTNAME], "
						+ "MetaModelInputColumn[PUBLIC.CUSTOMERS.CONTACTLASTNAME], "
						+ "MetaModelInputColumn[PUBLIC.CUSTOMERS.PHONE]]", job
						.getSourceColumns().toString());
	...
	public void testReadVariables() throws Exception {
		...
		DateMaskMatcherTransformer dateMaskMatcherTransformer = (DateMaskMatcherTransformer) tjbs
				.get(0).getConfigurableBean();
		assertEquals("[yyyy-MM-dd]",
				Arrays.toString(dateMaskMatcherTransformer.getDateMasks()));

		ConvertToDateTransformer convertToDateTransformer = (ConvertToDateTransformer) tjbs
				.get(1).getConfigurableBean();
		assertEquals("[yyyy-MM-dd]",
				Arrays.toString(convertToDateTransformer.getDateMasks()));
		assertEquals("2000-01-01",
				new SimpleDateFormat("yyyy-MM-dd")
						.format(convertToDateTransformer.getNullReplacement()));
	}
}
