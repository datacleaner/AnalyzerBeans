...
public class TimestampConverterTest extends TestCase {
	...
	public void testTransform() throws Exception {
		...
		MockInputColumn<Object> col = new MockInputColumn<Object>("my timestamps", Object.class);
		...
		assertEquals("2011-11-02 15:38", dateFormat.format(result[0]));
		
		result = trans.transform(new MockInputRow().put(col, 1234));
		assertEquals(1, result.length);
		assertEquals("1970-01-01 01:20", dateFormat.format(result[0]));
		...
		assertEquals(1, result.length);
		assertEquals("1970-01-21 01:00", dateFormat.format(result[0]));
	}
}
