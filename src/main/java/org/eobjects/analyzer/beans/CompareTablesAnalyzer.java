...
public class CompareTablesAnalyzer implements
		ExploringAnalyzer<TableComparisonResult> {

	private static final Logger logger = LoggerFactory
			.getLogger(TableComparisonResult.class);
	...
	public void run(DataContext dc) {
		...
		if (table1 == table2) {
			result = new TableComparisonResult(differences,
		...
		addDiff(differences, "remarks", table1.getRemarks(),
				table2.getRemarks());

		List<String> columnNames1 = CollectionUtils.list(table1
				.getColumnNames());
		List<String> columnNames2 = CollectionUtils.list(table2
				.getColumnNames());
		...
		for (String columnName : columnsOnlyInTable1) {
			TableDifference<String> diff = new TableDifference<String>(table1,
					table2, "unmatched column", columnName, null);
		...
		for (String columnName : columnsOnlyInTable2) {
			TableDifference<String> diff = new TableDifference<String>(table1,
					table2, "unmatched column", null, columnName);
			differences.add(diff);
		}
		...
		for (String columnName : columnsInBothTables) {
			...
			CompareColumnsAnalyzer analyzer = new CompareColumnsAnalyzer(
					column1, column2);
			analyzer.run(dc);
			ColumnComparisonResult columnComparisonResult = analyzer
					.getResult();
	...
	private <T> void addDiff(List<TableDifference<?>> differences,
			String valueName, T value1, T value2) {
		if (!CompareUtils.equals(value1, value2)) {
			TableDifference<T> diff = new TableDifference<T>(table1, table2,
